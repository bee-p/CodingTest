...
- 먼저 기존 배열의 크기(==size)를 저장한 뒤, 배열의 데이터들을 중복 제거한다.
- 중복 제거한 후의 배열의 크기가 size / 2를 한 값보다 같거나 크다면, 가져갈 수 있는 최대로 종류를 가질 수 있으므로 size / 2가 답이 된다.
- 그게 아니라면 중복 제거한 후의 배열의 크기가 답이 되는 식으로 풀이했다.
