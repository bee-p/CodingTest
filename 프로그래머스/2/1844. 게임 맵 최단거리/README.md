...
### 접근 방법
- BFS를 이용해 지나온 길의 값을 누적시켜 최단 거리를 구했다.
- 만약 목적지까지 도달했다면 이때까지 누적한 값이 저장되어 있을 것이고,<br>그게 아니라면 1이 그대로 저장되어 있을 것이므로 이 두 경우를 나누어 답을 출력했다.
