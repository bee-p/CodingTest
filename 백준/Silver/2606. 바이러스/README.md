...
### 접근 방법
- DFS, BFS 모두 구현이 가능한 문제인 것 같아 이 문제는 BFS로 구현했다
- 처음엔 BFS를 잘 구현한 것 같아 제출했으나, 4% 정도 채점되고 틀렸다고 나와서 조금 헤맸다….
  - 그런데 문제를 다시 보니 방향 상관없이 서로 연결만 되어있다면 바이러스 감염이 퍼지는 거였다
  - 나는 단방향으로 그래프를 구현하고 있었어서….. 만약 역방향으로 오는 바이러스가 있다면 이를 탐지하지 못해 채점 케이스를 통과하지 못했던 것이다!
    - ⇒ 그래서 연결 정보를 받을 때 정방향, 역방향 모두 정보를 넣어 그래프를 구현해 문제를 해결했다
