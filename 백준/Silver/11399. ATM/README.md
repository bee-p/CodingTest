...
***
### 접근 방법
처음에는 오름차순 정렬을 해서 차례대로 더하고, 각 더한 값들끼리 또 다시 더하면 될 것 같다는 생각이 들었다. <br>
그런데 변수를 여러 개 쓰지 않고, 이중 for문도 쓰지 않고 풀이할 수 있는 방법이 없을까 생각해봤다. (최대한 깔끔하게 풀고 싶었다...!!) <br>
그래서 아래와 같이 계산식을 그대로 나열해본 결과, N이 3일 경우
> a <br>
> (a) + b <br>
> (a + b) + c <br>

위와 같이 `(a*3) + (b*2) + (c*1)`이라는 규칙을 발견할 수 있었다. <br>
이를 위 문제와 맞게 정리하면 `(a*n) + (b*(n-1)) + (c*(n-2))`가 된다. <br>
따라서 이를 적용하여 문제를 풀었다.
