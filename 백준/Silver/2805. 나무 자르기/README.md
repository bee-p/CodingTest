...
### 접근 방법
- 최소 M미터 이상의 나무를 가져가기 위해서 설정할 절단기의 높이 H를 찾아야 하므로, 이 H를 찾는 과정을 이분탐색으로 구현했다.
- 우선 나무들의 높이는 내림차순으로 정렬하여 절단하고 남은 나무를 더하는 횟수를 최대한 줄일려고 했다.
- 탐색 범위는 low <= H <= high로 설정하였고, low과 high가 바뀌는 순간을 절단기의 최대 높이 값으로 봤다.
