...
### 접근 방법
- DFS와 재귀함수를 이용해 한 군집씩 상하좌우를 탐색하고, 체크하는 방식으로 구현했다.
- 그런데 온전히 내 힘으로 구현한 것이 아니라 이것저것 찾아보고 나서야 할 수 있었다.....
- DFS를 구현하는 방식을 어려워 하는 것 같다. 나중에 다시 풀어보자.

- **1/20 다시 풀이함!**
  - 예시 같은 것들을 찾아보지 않고 다시 풀이해봤다.
  - 이번에는 방문 배열을 따로 두지 않고, 방문한 배추는 없애는 것으로 표시하여 DFS 탐색을 진행했다.
  - doDfs()의 if 조건이 너무 긴 게 신경 쓰여 배열 안의 범위인지 체크하는 구문은 메소드로 따로 뺐다.
